...
CONFIG_FOLDER = r'C:\Users\KerenYlab\Desktop\Technion studies\Keren laboratory\python_playground\classifying-response-to-immunotherapy\XGBoost_facilities\configs'
CONFIG_PATH = join(CONFIG_FOLDER, 'dummy.yaml')
# CONFIG_PATH = join(CONFIG_FOLDER, 'dummy.yaml')
...
def main(dataset_config, xgboost_config, experiment_config):
    ...
    if experiment_config['save_model']:
        model.save_model_in_pkl(
            os.path.join(experiment_config['experiments_folder'], experiment_config['experiment_name']))
