...
class shapely_holder:
    def __init__(self, bst, dtrain, x, y):
        self.explainer = shap.TreeExplainer(bst)
        self.shap_values = self.explainer.shap_values(dtrain, check_additivity=False)
        self.X = x
        self.y = y
...
class Enhanced_XGboost:
    ...
    def train(self, rna_seq_dataset, verbose=False):
        ...
        for idx, (x_train, x_val, y_train, y_val, train_idxs, val_idxs) in enumerate(k_fold_validation):
            if verbose:
                print(f'Train XGBoost on fold number {idx + 1}|{self.k_folds}')
            # Trains current k-1 folds and validate by the k fold.
            dtrain = xgb.DMatrix(x_train, label=y_train, feature_names=rna_seq_dataset.gene_names)
            dval = xgb.DMatrix(x_val, label=y_val, feature_names=rna_seq_dataset.gene_names)
            bst = xgb.train(self.params, dtrain, 1, #self.num_round,
                            evals=[(dtrain, 'train'), (dval, 'validation')],
                            early_stopping_rounds=self.early_stopping_rounds,
                            verbose_eval=verbose)
            ...
            # shapely = shapely_holder(bst, dtrain, x_train, y_train)
    ...
    def inference(self, rna_seq_dataset, verbose=False):
        ...
        data_labels = np.array([p.response_label for p in rna_seq_dataset.cells_information_list])
        dcells = xgb.DMatrix(rna_seq_dataset.cells, label=data_labels, feature_names=rna_seq_dataset.gene_names)
        xgboost_patients_binary_preds = []
        xgboost_cells_binary_preds = []
        for idx, (bst, best_threshold_cell_probs, best_threshold_patient_probs) in enumerate(self.model_layers):
            if verbose:
